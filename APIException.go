...
type APIException struct {
	...
	Elements    []APIValidationElement `json:"Elements,omitempty"`
	HTTPStatus  int
}
...
func (e *APIException) GetUserDescription() string {
	if e.Elements != nil && len(e.Elements) > 0 && len(e.Elements[0].ValidationErrors) > 0 {
		messages := map[string]bool{}
		for _, m := range e.Elements[0].ValidationErrors {
			messages[m.Message] = true
		}
